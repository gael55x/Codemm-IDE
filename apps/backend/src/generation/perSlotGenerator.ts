...
function sanitizeJavaAssertEqualsExpectedStringWhitespace(testSuite: string): { testSuite: string; changed: boolean } {
  // Deterministic de-brittling: trim leading/trailing whitespace in the *expected* string literal
  // for `assertEquals(" ... ", actual)` patterns. This keeps the intent of most problems while
  // complying with our contract rule that forbids brittle whitespace literals by default.
  //
  // Note: This is intentionally conservative and only targets the common pattern generated by the LLM.
  let changed = false;
  const out = String(testSuite ?? "").replace(
    /(assertEquals\s*\(\s*")((?:[^"\\]|\\.)*)(")(\s*,)/g,
    (_m, p1: string, expected: string, p3: string, p4: string) => {
      const trimmed = String(expected).replace(/^[ \t]+|[ \t]+$/g, "");
      if (trimmed === expected) return `${p1}${expected}${p3}${p4}`;
      changed = true;
      return `${p1}${trimmed}${p3}${p4}`;
    }
  );
  ...
  try {
    ...
    if (raw.workspace && raw.reference_workspace) {
      ...
      if (hasBrittleWhitespaceStringExpectations(testSuite)) {
        const sanitized = sanitizeJavaAssertEqualsExpectedStringWhitespace(testSuite);
